...
| IntrStatus | 0x00F[31:16] | 0x3E | `0x0000` | write-1-to-clear; LinkChg on PHY write | none | `rtl8168_intr_emu.sv` |
...
| Config0/1/2 | 0x014 b1-3 | 0x51-0x53 | `00/00/80` | Cfg9346-gated | none | BRAM (Cfg9346 from SM) |
...
| ERIAR | 0x01D | 0x74 | `0x00000000` | flag-poll bit31; idx=[9:2] BE=[15:12] | 200-263 clk (~3-4 us) | `rtl8168_eriar_emu.sv` |
| GPHY_OCP | 0x02E | 0xB8 | `0x00000000` | flag-poll bit31; addr=[30:15] PHY=(addr-0xA400)/2 | 1200-1300 clk | `rtl8168_gphy_ocp_emu.sv` |
...
```
PM    0x40  id 01  next 0x50   PMC 0xC863  (D1+ D2+)
MSI   0x50  id 05  next 0x70   64-bit, Count 1/1
PCIe  0x70  id 10  next 0xB0   Express v2 Endpoint (LnkCap Gen1/x1, MPS 128B)
...
```
AER   0x100  id 0001  ver 1  next 0x140
VC    0x140  id 0002  ver 1  next 0x160
DSN   0x160  id 0003  ver 1  next 0x170   (per-unit EUI-64)
LTR   0x170  id 0018  ver 1  next 0x178
L1SS  0x178  id 001E  ver 1  next 0x000
```

The chain placement, next-pointers, and field values match a real RTL8168H.
